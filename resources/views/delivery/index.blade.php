                ...
                <table class="table delivery-table" id="deliveryTable">
                    ...
                    <tbody>
                        @foreach($deliveries as $delivery)
                            {{-- Main delivery row (clickable for accordion) --}}
                            <tr class="delivery-row"
                                ...
                                data-key="{{ $loop->index }}"
                                onclick="toggleDeliveryDetail(event, {{ $loop->index }})"
                                style="cursor:pointer;">
                                <td class="row-number">
                                    <span class="do-chevron" id="chevron-{{ $loop->index }}">▶</span>
                                    {{ $loop->iteration }}
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <div class="reseller-info-delivery">
                                        <div class="reseller-avatar-delivery">
                                            {{ strtoupper(substr($namaPelanggan !== '-' ? $namaPelanggan : 'N', 0, 1)) }}
                                        </div>
                                        <span>{{ $namaPelanggan }}</span>
                                    </div>
                                </td>
                                <td>{{ optional($delivery->pegawai)->nama_pg ?? 'Ibu Vivi Vandesa' }}</td>
                                <td>
                            ...
                            {{-- Accordion Detail Row: Invoice Items --}}
                            <tr class="delivery-detail-accordion" id="accordion-{{ $loop->index }}" style="display:none;">
                                <td colspan="8" style="padding:0; background:#F8F6FF; border-bottom: 2px solid #E9D5FF;">
                                    <div class="accordion-inner">
                                        {{-- Info bar --}}
                                        <div class="accordion-info-bar">
                                            <div class="accordion-info-item">
                                                <span class="accordion-info-lbl">📄 No Invoice</span>
                                                <a href="{{ route('invoice.show', $delivery->no_invoice) }}" class="accordion-inv-link" onclick="event.stopPropagation()">
                                                    {{ $delivery->no_invoice }}
                                                </a>
                                            </div>
                                            <div class="accordion-info-item">
                                                <span class="accordion-info-lbl">👤 Pelanggan / Reseller</span>
                                                <span class="accordion-info-val">{{ $namaPelanggan }}</span>
                                            </div>
                                            <div class="accordion-info-item" style="margin-left:auto;">
                                                <a href="{{ route('invoice.print', $delivery->no_invoice) }}" class="btn-acc-link" target="_blank" onclick="event.stopPropagation()">🖨️ Cetak Invoice</a>
                                                <a href="{{ route('delivery.show', $delivery->no_do) }}" class="btn-acc-link" onclick="event.stopPropagation()">📋 Detail DO</a>
                                            </div>
                                        </div>
                                            ...
                                            <table class="accordion-table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>Satuan</th>
                                                        <th>Harga (pcs)</th>
                                                        <th>Harga Reseller (pcs)</th>
                                                        <th>Konsinyasi (pcs)</th>
                                                        <th>Total</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach($delivery->invoice->detail as $di => $det)
                                                        @php
                                                            $hargaPcs      = $det->harga_perpcs ?? 0;
                                                            ...
                                                            $namaItem  = $det->item->nama_item ?? '-';
                                                            $parts     = explode(' - ', $namaItem, 2);
                                                            $itType    = $parts[0] ?? $namaItem;
                                                            $itDesc    = $parts[1] ?? '';
                                                        @endphp
                                                        <tr>
                                                            <td class="acc-center">{{ $di + 1 }}</td>
                                                            <td>{{ $itType }}</td>
                                                            <td>{{ $itDesc ?: $namaItem }}</td>
                                                            <td class="acc-center">
                                                                <span class="acc-qty-badge">{{ $qty }}</span>
                                                            </td>
                                                            <td class="acc-center">pcs</td>
                                                            <td class="acc-right">Rp {{ number_format($hargaPcs, 0, ',', '.') }}</td>
                                                            <td class="acc-right">Rp {{ number_format($hargaReseller, 0, ',', '.') }}</td>
                                                            <td class="acc-right acc-konsinyasi">Rp {{ number_format($konsinyasi, 0, ',', '.') }}</td>
                                                            <td class="acc-right acc-total">Rp {{ number_format($total, 0, ',', '.') }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                                <tfoot>
                                                    <tr>
                                                        <td colspan="3" class="acc-center" style="font-weight:800; letter-spacing:2px;">T O T A L</td>
                                                        <td class="acc-center">
                                                            <span class="acc-qty-badge" style="background:#7C5CDB;color:#fff;">{{ $accGrandQty }}</span>
                                                        </td>
                                                        <td></td>
                                                        <td></td>
                                                        <td></td>
                                                        <td></td>
                                                        <td class="acc-right" style="font-weight:800; font-size:14px; color:#7C5CDB;">
                                                            Rp {{ number_format($accGrandTotal, 0, ',', '.') }}
                                                        </td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        @else
                                            <div class="accordion-empty">📭 Tidak ada item detail dalam invoice ini</div>
                                        @endif
                                    </div>
                                </td>
                            </tr>
...
<style>
    ...
    @for($i = 0; $i < min($deliveries->count(), 20); $i++)
        .delivery-table tbody .delivery-row:nth-of-type({{ $i * 2 + 1 }}) {
    ...
    .delivery-row.row-expanded {
        background: #F5F3FF !important;
    }

    /* ===== Accordion Panel ===== */
    .delivery-detail-accordion {
        animation: slideDownAcc 0.25s ease-out;
    }
    ...
    .accordion-inv-link:hover { border-bottom-style: solid; }
    ...
    .btn-acc-link:hover { opacity: 0.85; color: #fff; }

    /* Accordion konsinyasi table */
    .accordion-table {
        width: 100%;
        border-collapse: collapse;
        font-size: 12px;
    ...
    .accordion-table tbody tr:hover td { background: #FAF8FF; }

    .accordion-table tfoot td {
        padding: 9px 9px;
        background: linear-gradient(135deg, #F5F3FF, #EDE9FE);
    ...
    .acc-konsinyasi { color: #7C5CDB; font-weight: 600; }
    .acc-total      { color: #059669; font-weight: 700; }
...
<script>
    // ===== Accordion toggle =====
    function toggleDeliveryDetail(event, idx) {
        // Don't toggle if clicking action buttons
        if (event.target.closest('.action-buttons-delivery') || event.target.closest('form')) return;

        const accordion = document.getElementById('accordion-' + idx);
        const chevron   = document.getElementById('chevron-' + idx);
        const mainRow   = document.querySelector('[data-key="' + idx + '"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterTable() {
            ...
            rows.forEach(function(row) {
                const idx      = row.getAttribute('data-key');
                ...
                // Also hide/show the accordion row
                const accordionRow = document.getElementById('accordion-' + idx);

                if (matchesSearch && matchesStatus) {
                    row.style.display = '';
                    visibleCount++;
                    counter++;
                    // Update row number (only the number part, keep chevron)
                    const rn = row.querySelector('.row-number');
                    if (rn) rn.childNodes[rn.childNodes.length - 1].textContent = ' ' + counter;
                } else {
                    row.style.display = 'none';
                    // Also close accordion when row hidden
