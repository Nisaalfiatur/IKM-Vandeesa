    ...
    <div class="form-card-body">
        <form action="{{ route('delivery.store') }}" method="POST">
            ...
            <div class="form-grid">
                    ...
                    <select name="no_invoice" id="invoiceSelect" class="{{ $errors->has('no_invoice') ? 'is-invalid' : '' }}">
                        ...
                        @foreach ($invoices as $invoice)
                            <option value="{{ $invoice->no_invoice }}" {{ old('no_invoice') == $invoice->no_invoice ? 'selected' : '' }}>
                                {{ $invoice->no_invoice }} - {{ $invoice->pelanggan->nama ?? '-' }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label>
                        🏪 Reseller <span class="required">*</span>
                    </label>
                    <select name="id_reseller" class="{{ $errors->has('id_reseller') ? 'is-invalid' : '' }}">
                        <option value="">-- Pilih Reseller --</option>
                        @foreach ($resellers as $reseller)
                            <option value="{{ $reseller->id_reseller }}" {{ old('id_reseller') == $reseller->id_reseller ? 'selected' : '' }}>
                                {{ $reseller->id_reseller }} - {{ $reseller->nama }} ({{ $reseller->nama_brand }})
                            </option>
                        @endforeach
                    </select>
                    @if ($errors->has('id_reseller'))
                        <span class="field-error">⚠ {{ $errors->first('id_reseller') }}</span>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="id_pegawai" class="{{ $errors->has('id_pegawai') ? 'is-invalid' : '' }}">
                    ...
                    @if ($errors->has('id_pegawai'))
                        <span class="field-error">⚠ {{ $errors->first('id_pegawai') }}</span>
                    @endif
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updatePreview() {
            ...
            prevPelanggan.textContent = invoice.pelanggan ? invoice.pelanggan.nama : '-';
