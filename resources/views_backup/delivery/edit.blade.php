    ...
    <div class="card">
        <form action="{{ route('delivery.update', $delivery->no_do) }}" method="POST">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    ...
                    <select name="no_invoice" id="no_invoice" required
                        ...
                        @foreach($invoices as $invoice)
                            <option value="{{ $invoice->no_invoice }}" {{ old('no_invoice', $delivery->no_invoice) == $invoice->no_invoice ? 'selected' : '' }}>
                                {{ $invoice->no_invoice }} - {{ $invoice->pelanggan->nama ?? 'N/A' }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Reseller --}}
                <div>
                    <label for="id_reseller"><strong>Reseller</strong></label>
                    <select name="id_reseller" id="id_reseller" required
                            style="{{ $errors->has('id_reseller') ? 'border-color: #EF4444;' : '' }}">
                        <option value="">-- Pilih Reseller --</option>
                        @foreach($resellers as $reseller)
                            <option value="{{ $reseller->id_reseller }}" {{ old('id_reseller', $delivery->id_reseller) == $reseller->id_reseller ? 'selected' : '' }}>
                                {{ $reseller->id_reseller }} - {{ $reseller->nama }} ({{ $reseller->nama_brand }})
                            </option>
                        @endforeach
                    </select>
                    @if($errors->has('id_reseller'))
                        <span style="color: #EF4444; font-size: 12px; margin-top: -10px; display: block;">{{ $errors->first('id_reseller') }}</span>
                    @endif
                </div>

                {{-- Pegawai --}}
                <div>
                    <label for="id_pegawai"><strong>Pegawai (Pengirim)</strong></label>
                    <select name="id_pegawai" id="id_pegawai" required
                            style="{{ $errors->has('id_pegawai') ? 'border-color: #EF4444;' : '' }}">
                        <option value="">-- Pilih Pegawai --</option>
                        @foreach($pegawais as $pegawai)
                            <option value="{{ $pegawai->id_pegawai }}" {{ old('id_pegawai', $delivery->id_pegawai) == $pegawai->id_pegawai ? 'selected' : '' }}>
                                {{ $pegawai->id_pegawai }} - {{ $pegawai->nama_pg }} ({{ $pegawai->jabatan }})
                            </option>
                        @endforeach
                    </select>
                    @if($errors->has('id_pegawai'))
                        <span style="color: #EF4444; font-size: 12px; margin-top: -10px; display: block;">{{ $errors->first('id_pegawai') }}</span>
                    @endif
                </div>
...
<script>
    ...
    const invoices = @json($invoices->map(function($inv) {
        return [
            'no_invoice' => $inv->no_invoice,
            'pelanggan' => $inv->pelanggan->nama ?? 'N/A',
            'tanggal' => $inv->tanggal,
            'total_harga' => $inv->total_harga,
            'items' => $inv->detail->map(function($d) {
                return [
                    'nama' => $d->item->nama_item ?? 'N/A',
                    'qty' => $d->quantity,
